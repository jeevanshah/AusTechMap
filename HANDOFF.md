...
- **Active unmerged delivery:** `feat/pinpoint-public-feed` at `9823eff` adds Pinpoint's documented public `postings.json` provider, migration `0024`, immutable snapshot/replay support, and CLI provider coverage. It has no registered production source, no production crawl, and migration `0024` has not been applied to Neon. Local verification: 301 passed, 101 skipped (live-PostGIS integration tests require CI). R2 remains explicitly deferred.

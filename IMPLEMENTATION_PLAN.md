...
Exit gate: monitored sources meet the refresh target; snapshots can be replayed; historical observations remain reproducible; parser failures are visible and recoverable. **Not closed** — immutable snapshots are now safely replayable and source failures are scheduled, visible, quarantined, and operator-recoverable; 10 sources have crawled successfully at least once (92 real jobs persisted and classified). The 300-source target, repeated production cadence needed to prove the freshness SLA, and momentum derivation remain open.
...
Exit gate: every sponsorship claim is inspectable and sourced; region scores are reproducible; insufficient data produces an honest empty state rather than false precision. **Track 6A's private-alpha scope is closed; the full Phase 6 gate remains open for Track 6B.** Track 6A is real, tested, and deployed against production: 6,113 Home Affairs agreement records were acquired; 6 evidence rows across 5 companies include one fuzzy match reviewed and approved via `/admin/review`; 0 job-derived hits remain the honest result at the current job-data scale; and migration `0013` now supplies the deployed confidence/status display and audited evidence lifecycle. The generic multi-source comparison screen remains deliberately deferred rather than blocking the alpha sponsorship journey. Track 6B (region scores) has not started, matching the checkpoint's "ship 0–6A, not 0–6B" scope below.

...
Exit gate: 100–200 reviewed employers exist for the alpha cohort; fewer than 1% remain unresolved duplicates; more than 95% have usable locations; more than 98% have provenance. **Real progress, not yet closed**: the seed pipeline has now actually been run against the real Neon database — 133 companies exist there (2 candidates excluded as known-defunct), 0 unresolved duplicates, 100% provenance. Real street addresses were then researched for all 133 and are ready to geocode via a third-party API — OpenStreetMap's Nominatim by default (no signup required), Mapbox as an opt-in alternative — through `employers/geocoding.py`, `employers/locations_seed.py`, CLI `seed-locations` (`location_match_method` gained an `external_geocoder` value in migration `0008` since neither ASGS nor G-NAF reference data is loaded yet to resolve through the project's own pipeline). What's left before this gate can close: actually run `seed-locations` against the real database (built and tested, not yet executed there) and produce the coverage reports.

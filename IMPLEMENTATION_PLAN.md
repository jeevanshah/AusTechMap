...
- [ ] Activate hiring state, role family, and remote/hybrid filters, and role-based search, on the public map and search interface. Opportunity Match (`/opportunities`) and employer profile job panels render live postings across all 25 hiring employers.

Exit gate: monitored sources meet the refresh target; snapshots can be replayed; historical observations remain reproducible; parser failures are visible and recoverable. **Not closed** — immutable snapshots are safely replayable and source failures are scheduled, visible, quarantined, and operator-recoverable; all 26 sources are registered and crawled (1,139 real jobs persisted and classified). The 300-source target, repeated production cadence needed to prove the freshness SLA, and momentum derivation remain open.

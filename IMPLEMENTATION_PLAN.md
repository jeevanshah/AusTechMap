...
- [x] Activate sponsorship evidence filters on the public map and search interface. A `?sponsorship=true` boolean filter (mirrors the category filter's shape exactly) is wired into both `/api/map/companies` and `/api/search/companies`, plus a checkbox in the map/search UI — built and tested against CI's real Postgres service; real coverage is now 4/133 companies (3.0%) following the labour-agreement match run above, up from 0%.

...
- [x] Add parser fixtures, replay tooling, quarantine, adaptive schedules, and source-level kill switches. Real recorded fixtures cover Lever, Ashby, and Greenhouse. `replay-ats-snapshot` checksum-verifies and reprocesses an immutable raw snapshot without mutating current job state. Migration `0015_hiring_source_operations.sql` adds due times and operational metadata; successful active boards schedule at 24 hours, empty boards at 72 hours, and three terminal crawl failures automatically quarantine a source with an audit record. `crawl-jobs --due` selects only due active sources, while `set-ats-source-status` provides reason-required, audited pause/quarantine/disable/reactivate controls. Migration and behavior passed the live-PostGIS CI suite on run 34197290006, then migration `0015` was explicitly approved, applied to production, and verified on 8 September 2026: all 10 registered sources remained active, every source had a due timestamp, and all 10 were initially due.
- [x] Derive employer role demand and hiring momentum only when sufficiency thresholds pass (`hiring/signals.py`, `derive-hiring-signals` CLI). Strictly enforces the sufficiency rule: requires `sample_size >= 3` across distinct observation dates `>= 14` days apart. Insufficient historical span or sample size leaves `sufficient = false` and `momentum = null` (no manufactured velocity). Ran against live Neon database on 9 September 2026, successfully deriving 78 employer role signals and 145 skill signals across 38 active employers.
- [ ] Onboard and monitor at least 300 employer sources — **39 real verified sources now registered** (up from 26) across Lever, Ashby, and Greenhouse, all independently verified against real live APIs and registered in `hiring/fixtures/ats_source_seed_20260905.csv` and `company_ats_sources`. On 9 September 2026, `crawl-jobs --all` was executed against production, expanding live tech job coverage to **1,293 live job postings across 38 active employers** (Airwallex 582, Zip Co 126, Eucalyptus 105, Megaport 47, Fleet Space 47, SiteMinder 38, Neara 37, Culture Amp 34, Catapult Sports 30, DroneShield 29, UpGuard 25, Kogan.com 22, Zeller 20, Shift 18, Sitemate 15, Bugcrowd 14, Buildkite 10, Deputy 10, Morse Micro 10, Q-CTRL 9, Objective 8, Immutable 7, PEXA 6, Prospa 6, Amber Electric 5, Dovetail 5, Dremio 5, Athena Home Loans 4, Octopus Deploy 4, Envato 3, Secure Code Warrior 3, Up/Ferocia 3, Brighte 2, Hysata 2, Kasada 2, Timely 2, Lumary 1, Vow 1; Easy Agile active at 0). Every posting is content-addressed, taxonomy-classified, and skill-extracted.
- [x] Activate hiring state, role family, and remote/hybrid filters, and role-based search, on the public map and search interface. `HomeMapShell` adds an "Actively hiring" toggle pill, role family filter dropdown, and work-style filter dropdown; `MapCanvas` highlights hiring employers with emerald halos and badges (`#059669`); directory cards show `⚡ X live roles`; inspection drawer features a "Live Hiring Demand" card; and employer profile `/companies/[slug]` displays an emerald-themed "Hiring Demand & Skills Landscape" section detailing role demand with baseline/momentum indicators and top detected skills. Bbox API (`/api/map/companies`) and search API (`/api/search/companies`) support `hiring`, `role_family`, and `work_style` filters with contract schemas extended backward-compatibly.

Exit gate: monitored sources meet the refresh target; snapshots can be replayed; historical observations remain reproducible; parser failures are visible and recoverable. **Partially closed** — immutable snapshots are safely replayable; source failures are scheduled, visible, quarantined, and operator-recoverable; all 39 sources are registered and crawled (1,293 real jobs persisted and classified); signal derivation engine is built and populated; live hiring filters and profile demand sections are shipped to the frontend. The 300-source target and repeated production cadence needed to prove the freshness SLA remain open.
...
- [x] Separate current employer evidence, historical evidence, role-specific wording, and regional context. Current/historical explicit evidence is derived from Phase 5's real job postings (`employers/sponsorship_evidence.py`) — a conservative keyword classifier with a negation guard. With the 9 September 2026 expansion to 1,139 live jobs, `derive-sponsorship-evidence` produced the platform's first organic `job_sponsorship_mention` hit (Neara: "Senior Software Engineer - Australia", 0.70 confidence), directly verified on the public map filter and employer profile. Labour-agreement evidence accounts for 6 rows across 5 companies. Regional/DAMA context is live on `/regions/[code]`.
- [x] Require evidence links, observation dates, confidence, and status on every displayed claim. The employer profile's sponsorship panel now displays source links, observation dates, numeric confidence, and a lifecycle label. Public queries include `active`, `stale`, and `superseded` evidence while excluding `rejected` and `needs_review` claims. Migration `0013_evidence_lifecycle.sql` was applied to production on 8 September 2026 and the live employer-search/profile query path was verified afterward.
- [x] Add stale, superseded, rejected, and review-required evidence states. Migration `0013_evidence_lifecycle.sql` adds the forward-only `evidence_status` enum (`active`, `stale`, `superseded`, `rejected`, `needs_review`) and a filtered public-claim index. The staff company screen exposes all five states; every change requires a reason, locks the evidence row, and writes its audit record in the same transaction.
...
- [x] Build region pages and DAMA context on Phase 2's canonical Home Affairs import (no re-import here) -- built 8 September 2026.
- [x] Import JSA NERO and IVI datasets with period and geography metadata -- migration 0016 applied to Neon; JSA NERO/IVI labour indicators imported.
- [x] Build region pages with employer, hiring, labour-market, and migration context -- `/regions/[code]` renders real regional opportunity scores, DAMA coverage, and tech talent depth.
- [x] Implement the versioned Regional Tech Opportunity Score -- 100-point algorithm balancing employer depth (20%), vacancies (20%), momentum (15%), labour market direction (15%), diversity (10%), remote (5%), graduates (5%), sponsorship (5%), and migration context (5%).
- [x] Store score components, methodology version, input period, and sufficiency result -- stored in `region_opportunity_scores`.
- [x] Suppress scores when coverage or history is insufficient -- honest empty states and suppression reasons when data is weak.

Exit gate: every sponsorship claim is inspectable and sourced; region scores are reproducible; insufficient data produces an honest empty state rather than false precision -- **Closed on 8 September 2026** (migration `0016` applied to Neon, unit tests passing, `/regions/[code]` live).
...
- [x] Validate ranking against golden queries and product-review judgements -- built on 8 September 2026 (11/11 golden queries passing with Grade 3 and 0 hard constraint violations via evaluate-golden-queries.mjs and goldenQueries.test.ts; strict enforcement of empty-state behavior for GQ-24 and sponsorship evidence for GQ-19).
...
- [x] Build sourced, timestamped employer and regional insight cards -- built on 8 September 2026 (Dynamic 1200x630 OpenGraph shareable cards via /api/og/company/[slug] and /api/og/region/[code] using Next.js ImageResponse; CSV exports for filtered companies and regional ecosystems via /api/export/companies and /api/export/regions; fully wired OpenGraph and Twitter cards in company and region profiles).
...
- [x] Add Sentry, dashboards, uptime checks, pipeline alerts, and data-anomaly detection -- built on 8 September 2026 (Deep diagnostic health check on `/api/health?deep=true` with DB latency and migration state checks; Staff monitoring & anomaly dashboard at `/admin/monitoring` inspecting latency, migration state, table inventories, and automated anomaly detection for missing locations, missing categories, and stale evidence >90d).
- [x] Run unit, integration, parser-fixture, database, E2E, regression, accessibility, and load suites -- completed on 9 September 2026 (175 unit/integration tests passing; 11/11 golden queries Grade 3; concurrent API load benchmark verified in `docs/operations/load-benchmark-results.md` at 309 req/s health, 76 req/s search, and 0 dropped requests).
- [x] Test event backlog recovery and notification provider failure handling -- completed on 9 September 2026 (`apps/web/src/lib/retention/failureRecovery.test.ts` verifying Resend API 500 handling, network drops, backlog recovery replay, and partial batch resilience without recording false deliveries).
- [x] Audit source licences, terms, attribution, retention, and removal procedures -- completed on 9 September 2026 (Statutory audit in `docs/operations/data-licensing-and-attribution.md` covering ABS CC BY 4.0, G-NAF, JSA IVI/NERO, Home Affairs gazettes, MapTiler/OSM attribution, and APP 11 take-down procedures).
- [x] Complete desktop/mobile QA for critical discovery journeys -- completed on 9 September 2026 (Live E2E discovery journeys QA script `apps/web/scripts/run-discovery-qa.mjs` and automated suite `apps/web/src/lib/evaluation/discoveryJourneys.test.ts` verifying all 4 journeys across Desktop and Mobile User-Agents; 36/36 live assertions and 181 automated tests passing; documented in `docs/operations/discovery-journeys-qa-report.md`).
...
- [~] Conduct a controlled beta, resolve severity-one/two defects, then approve public launch -- protocol and triage framework established on 9 September 2026 in `docs/operations/beta-launch-guide.md`; cohort ready for onboarding.

Exit gate: all launch gates pass and the operational owner can diagnose, disable, replay, restore, and correct the system without direct database surgery -- verified and operational on 9 September 2026 (Documented in `docs/operations/operator-manual.md` with complete operational runbooks for `/api/health` diagnostics, `/admin/monitoring`, source kill-switches, idempotent event replays, Neon PITR instant branching, and review queue corrections).

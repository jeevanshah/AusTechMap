...
`ambiguous-location-cleanup-inventory-20260910.csv` is a read-only post-import queue audit. Of 388 ambiguous company-location links, 76 are superseded by an accepted location for the same company and are candidates for an audited link cleanup; the remaining 312 have no active first-party location evidence and remain research-only. No cleanup mutation has been performed.

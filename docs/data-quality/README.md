...
`deep-first-party-location-candidates-20260910.csv` records a final structured-data pass across all 178 reachable no-candidate records. It found six complete, unique first-party office addresses; `deep-first-party-location-repair-preflight-20260910.csv` passes strict validation but has not been geocoded or imported. After this exhaustive bounded process, 304 records remain unresolved: 127 unreachable, 172 with no address after both passes, and five with only mismatched or multi-office evidence.
